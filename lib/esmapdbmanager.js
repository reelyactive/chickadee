...
const DEFAULT_DATABASE_NAME = 'documents';
...
class ESMapDBManager {
  ...
  constructor(options) {
    options = options || {};
    options.databaseName = options.databaseName || DEFAULT_DATABASE_NAME;

    let persistentLocation = DEFAULT_DATA_FOLDER + '/' + options.databaseName;
