  ...
  retrieve(id, type, directory, tag, callback) {
    ...
    if(directory) {
      lookupDevicesByDirectory(directory, self.database, function(deviceIds) {
        retrieveContext(deviceIds, self.barnacles, self.database, callback);
      });
    }
    else if(tag) {
 ...
 * @param {Array} deviceIds The devices to retrieve.
 ...
 */
function retrieveContext(deviceIds, barnacles, database, callback) {
  let hasDevices = Array.isArray(deviceIds) && (deviceIds.length > 0);

  if(!hasDevices) {
    return callback(404);
  }

  barnacles.retrieveContext(deviceIds, null, function(devices) {
    if(devices) {
...
function lookupDevicesByDirectory(directory, database, callback) {
  ...
  database.find(query, { _id: 1 }, function(err, associations) {
    ...
    associations.forEach(function(association) {
      let idElements = association._id.split(SIGNATURE_SEPARATOR);
      deviceIds.push({ deviceId: idElements[0],
                       deviceIdType: idElements[1] || 0 });
    });
