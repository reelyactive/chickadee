...
function retrieveAssociation(req, res) {
  ...
  switch(req.accepts(['json', 'html'])) {
    ...
    default:
      ...
      req.chickadee.associations.retrieve(req, id, type, null, rootUrl,
                                          queryPath,
...
function replaceAssociation(req, res) {
  ...
  let rootUrl = req.protocol + '://' + req.get('host');
  let queryPath = req.originalUrl;
  req.chickadee.associations.replace(req, id, type, url, directory, tags,
                                     position, rootUrl, queryPath,
                                     function(response, status) {
    res.status(status).json(response);
  });
}
...
function removeAssociation(req, res) {
  ...
  let rootUrl = req.protocol + '://' + req.get('host');
  let queryPath = req.originalUrl;
  req.chickadee.associations.remove(req, id, type, null, rootUrl, queryPath,
                                    function(response, status) {
    res.status(status).json(response);
  });
}
