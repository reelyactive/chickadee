...
function retrieveAssociation(req, res) {
  ...
  switch(req.accepts(['json', 'html'])) {
    ...
    default:
      ...
      associations.retrieve(req, id, type, null, rootUrl, queryPath,
...
function replaceAssociation(req, res) {
  ...
  let queryPath = req.originalUrl;
  let associations = req.chickadee.associations;
  associations.replace(req, id, type, url, directory, tags, position, rootUrl,
...
function removeAssociation(req, res) {
  ...
  let queryPath = req.originalUrl;
  let associations = req.chickadee.associations;
  associations.remove(req, id, type, null, rootUrl, queryPath,
                      function(response, status) {
    res.status(status).json(response);
  });
}
