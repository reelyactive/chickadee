...
var SNIFFYPEDIA_ROOT_URL = 'http://sniffypedia.org/';
...
function makeExternalAssociation(device, rootUrl) {
    ...
    switch(oui36) {
      case "001bc5094": // reelyActive
        if(device.identifier.value.substr(9,2) === '01') {
          return "http://reelyactive.com/products/ra-t411/";
        }
      default: // Unknown
      ...
      switch(serviceData.uuid) {
        case "feaa":  // Eddystone (Google)
          if(serviceData && serviceData.eddystone &&
            ...
            return "http://reelyactive.com/metadata/eddystone.json";
          }
        case "fed8":  // Physical Web (Google)
          if(serviceData && serviceData.uriBeacon &&
            ...
            return "http://reelyactive.com/metadata/physicalweb.json";
          }
      }
    }
  }
  ...
  if(hasAssociationIds) {
    ...
    for(var cId = 0; cId < device.associationIds.length; cId++) {
      var associationId = device.associationIds[cId];
      if(associationId.length === 32) {
        if(sniffypedia.index.ble.uuid128.hasOwnProperty(associationId)) {
          return assembleUrl(sniffypedia.index.ble.uuid128[associationId],
                             rootUrl);
        }
        else if(sniffypedia.index.ble.iBeacons.hasOwnProperty(associationId)) {
          return assembleUrl(sniffypedia.index.ble.iBeacons[associationId],
                             rootUrl);
        }
      }
      else if(associationId.length === 4) {
        if(sniffypedia.index.ble.uuid16.hasOwnProperty(associationId)) {
          return assembleUrl(sniffypedia.index.ble.uuid16[associationId],
                             rootUrl);
        }
        else if(sniffypedia.index.ble.companyIdentifiers.hasOwnProperty(
                                                             associationId)) {
          return assembleUrl(
            sniffypedia.index.ble.companyIdentifiers[associationId], rootUrl);
        }
      }
    }
  }
  ...
  if(advHeader) {
    ...
    if(type === "SCAN_REQ") { // Curious device
      return "http://reelyactive.com/metadata/curious.json";
    }
    else { // Unknown Bluetooth Smart device
      return "http://reelyactive.com/metadata/bluetoothsmart.json";
    }
  }
}
