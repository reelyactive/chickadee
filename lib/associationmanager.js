...
var UUID_URL_BASE_URL = 'https://myjson.info/s/';
...
AssociationManager.prototype.appendAssociations = function(state, associations,
  ...
  for(var deviceId in devices) {
    ...
    if(lookupExists) {
      ...
      if(lookup[deviceId].position) {
        device.position = lookup[deviceId].position;
      }
    }
...
function makeExternalAssociation(device, rootUrl) {
    ...
    switch(oui36) {
      case "001bc5094": // reelyActive
        if(device.identifier.value.substr(9,2) === '01') {
          return SNIFFYPEDIA_ROOT_URL + 'Product/reelyActive_RA-T411/';
        }
      default: // Unknown
  ...
  if(advHeader) {
    ...
    if(type === "SCAN_REQ") { // Curious device
      return SNIFFYPEDIA_ROOT_URL + 'Product/Any_Curious-Device/';
    }
    else { // Unknown Bluetooth Low Energy device
      return SNIFFYPEDIA_ROOT_URL + 'Organization/Bluetooth_SIG_Inc/';
    }
  }
}
 ...
 * @param {Array} position The position to associate.
 * @param {function} callback Function to call on completion.
 */
