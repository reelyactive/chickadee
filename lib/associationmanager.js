 ...
 * @param {Object} identifiers Identifier(s) to associate.
 * @param {String} url URL to associate.
 * @param {function} callback Function to call on completion.
 */
AssociationManager.prototype.replace = function(id, identifiers, url,
                                                callback) {
  var self = this;
  var association = {};

  if(identifiers) {
    association.identifiers = identifiers;
  }
  if(url) {
    association.url = url;
  }
...
AssociationManager.prototype.retrieve = function(id, parameters, callback) {
  ...
  if(id) { query = { _id: id }; }
  else if(parameters.value) { // TODO: remove if parametric query is dropped
    // TODO: should check if value and txAdd match for Bluetooth
    query = { $or: [ { "identifiers.value": parameters.value },
                     { identifiers: parameters.value } ] };
  }
