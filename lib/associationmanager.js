...
AssociationManager.prototype.addAssociations = function(req, state, callback) {
  ...
  self.db.find({ _id: { $in: ids } }, function(err, associations) {
    ...
    for(var deviceId in devices) {
      ...
      // Otherwise, add associations based on embedded ID
      else {
        ...
        if(advDataExists) {
          ...
          if(uuid) {
            ...
            if(lookupExists) {
              device.url = lookup[uuid].url || DEFAULT_DEVICE_URL;
              device.tags = lookup[uuid].tags || DEFAULT_DEVICE_TAGS;
            }
          }
        }
    ...
    switch(oui36) {
      case "001bc5094": // reelyActive
        if(device.identifier.value.substr(9,2) === '01') {
          device.url = "http://reelyactive.com/products/ra-t411/";
        }
        else {
  ...
  if(advData) {
    ...
    switch(uuid) {
      case "7265656c794163746976652055554944":  // RA-R436
        device.url = "http://reelyactive.com/products/ra-r436/";
        return;
      case "7265656c7941707020416e64726f6964":  // reelyApp Android
        device.url = "http://reelyactive.com/metadata/reelyApp-Android.json";
        return;
      case "7265656c7941707020666f7220694f53":  // reelyApp iOS
        device.url = "http://reelyactive.com/metadata/reelyApp-iOS.json";
        return;
      ...
      case "52052c11e701478299f58ce88dbb1500":  // Schlage ENGAGE Lock
        ...
        return;
      case "d2d3f8ef9c994d9ca2b391c85d44326c":  // Nest Cam
        device.url = "http://reelyactive.com/metadata/nestcam.json";
        return;
      case "f02adfc026e711e49edc0002a5d5c51b":  // Flic
        device.url = "http://reelyactive.com/metadata/flic.json";
        return;
      case "0f3e":  // TrackR
    ...
    if(manufacturerSpecificData) {
      ...
      if(iBeacon) {
        switch(iBeacon.licenseeName) {
          ...
          case "eNote":
            ...
            return;
          case "The Bubbles Company inc.":
            device.url = "http://reelyactive.com/metadata/bubblescompany.json";
            return;
          case "XY Find It":
            device.url = "http://reelyactive.com/metadata/xyfindit.json";
            return;
          default:
      ...
      var companyIdentifierCode =
        manufacturerSpecificData.companyIdentifierCode;
...
function extractAssociationIds(devices) {
  ...
  for(var deviceId in devices) {
    ...
    if(hasAssociationIds) {
      ids = device.associationIds;
    }
    else {
      ids = reelib.tiraid.getAssociationIds(device);
    }
