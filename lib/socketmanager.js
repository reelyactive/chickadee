...
class SocketManager {
  ...
  constructor(options, io, barnacles, chimps, context) {
    ...
    if(io) {
      io.of(CONTEXT_ROUTE_REGEX).on('connection', (socket) => {
...
function handleBarnaclesEvents(barnacles, activeNamespaces, context) {
  ...
  barnacles.on('encrypted', (encrypted) => {
    handleEncryptedEvent(encrypted, activeNamespaces);
  });
}
...
function handleDynambEvent(dynamb, activeNamespaces, context) {
  ...
  if(activeNamespaces.has(deviceRoute)) {
    ...
    if(isPossibleContextChange) {
      context.retrieve(dynamb.deviceId, dynamb.deviceIdType, null, null, null,
                       (status, response) => {
        activeNamespaces.get(deviceRoute).emit('devices', response.devices);
      });
    }
...
/**
 * Handle an encrypted event from barnacles.
 * @param {Object} encrypted The encrypted event.
 * @param {Map} activeNamespaces The active namespaces to update.
 */
function handleEncryptedEvent(encrypted, activeNamespaces) {
  let signature = encrypted.deviceId + '/' + encrypted.deviceIdType;
  let deviceRoute = CONTEXT_ROUTE + DEVICE_ROUTE + '/' + signature;

  if(activeNamespaces.has(deviceRoute)) {
    activeNamespaces.get(deviceRoute).emit('encrypted', encrypted);
  }
}
